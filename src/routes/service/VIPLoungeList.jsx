...
import AddVIPLounge from './AddVIPLounge';//添加贵宾厅
import UpdateVIPLounge from './UpdateVIPLounge';//修改贵宾厅
...
class VIPLoungeList extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            visible:false,
    ...
    getInitList=(page,rows)=> {
        ...
        $.ajax({
            ...
            data: { access_token: User.appendAccessToken().access_token,page:page,rows:rows,typeId:1},
        ...
        const columns = [
            {
                ...
                render(text, record,index) {
                    return (
                        <div className="order">
                            <a href="javascript:;" onClick={_this.showModal0.bind(_this,record)}  style={{ marginRight: 10,color:'#4778c7' }}>修改</a>
        ...
        return (
            <div>
                <div className="sub-tit">
                    <p className="f16 fl">贵宾厅列表</p>
                    <button className="btn fr" onClick={this.showModal}>增加贵宾厅</button>
                </div>
                <p style={{ marginTop: 28 }}>共<span>{this.state.serviceListLength}</span>个贵宾厅</p>
                <div className="search-result-list">
                    <Table key='t1' columns={columns} dataSource={this.state.serviceList} pagination={pagination} className="serveTable" />
                ...
                <div className="servive-modal">
                     <Modal title="新增贵宾厅"
                         ...
                         >
                         <div>
                             <AddVIPLounge handleClick={this.handleClick}  serviceTypeAllocationId={this.props.key}/>
                         </div>
                     </Modal>
                 </div>
                 ...
                 <div className="servive-modal">
                     <Modal title="修改贵宾厅"
                         ...
                         onOk={this.handleOk0}
                         confirmLoading={this.state.confirmLoading0}
                         onCancel={this.handleCancel0}
                         >
                         <div>
                             <UpdateVIPLounge handleClick={this.handleClick0} record={this.state.record} />
                         </div>
                     </Modal>
                 </div>
                 <div className="servive-modal">
...
VIPLoungeList = Form.create()(VIPLoungeList);

export default VIPLoungeList;
