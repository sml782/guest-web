...
class VIPShuttleBusProduct extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            value: 1,
    ...
    getInitialValue=(page,rows)=>{
        ...
        $.ajax({
            ...
            data: {access_token: User.appendAccessToken().access_token,page:page,rows:rows,typeId:2,protocolProductId:_this.props.protocolProductId},
        ...
        const columns = [
            {
                ...
                render(text, record,index) {
                    return (
                        <div className="order">
                            <Checkbox onChange={_this.availableChange.bind(_this,record,index)} checked={true} checked={record.state1}></Checkbox>
                        </div>
                    )
                }
            }
        ...
        return (
            <div>
                <div className="sub-tit">
                    <p className="f16 fl">VIP摆渡车配置</p>
                    <button className="btn-small fr" onClick={this.saveBtn}>保&nbsp;&nbsp;存</button>
                </div>
                <p style={{ marginTop: 28 }}>共<span>{this.state.listDataLength}</span>个VIP摆渡车</p>
...
VIPShuttleBusProduct = Form.create()(VIPShuttleBusProduct);

export default VIPShuttleBusProduct;
